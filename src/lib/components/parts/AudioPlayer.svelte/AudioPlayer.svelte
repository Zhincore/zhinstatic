...
<div class={className}>
  ...
  <Visualizer class="my-4 h-32 w-full text-accent-400" {audio} mode={$vismode} />
  ...
  <div class="flex min-w-[256px] items-center" role="region" aria-label="Audio Player">
    ...
    >
      <div
        class="absolute inset-y-0 h-full rounded-lg bg-zinc-500"
        style:left={`${range[0] * 100}%`}
        style:width={`${range[1] * 100}%`}
      />
      <div class="relative h-full rounded-lg bg-accent-400" style:width={`${(currentTime / duration) * 100}%`} />
    </div>
